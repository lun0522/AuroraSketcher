...
using Point = DistanceField::Point;

static const Point inside = { 0, 0 };
static const Point outside = { 16384, 16384 };

DistanceField::DistanceField(const int width, const int height):
imageWidth(width), imageHeight(height),
gridWidth(width + 2), gridHeight(height + 2) {
    numPoint = gridWidth * gridHeight; // include padding
    grid.points = (Point *)malloc(numPoint * sizeof(Point));
}

void DistanceField::generate(unsigned char* image) {
...
inline Point DistanceField::get(const int x, const int y) {
    return grid.points[(y + 1) * gridWidth + (x + 1)];
}

inline void DistanceField::put(const int x, const int y, const Point &p) {
    grid.points[(y + 1) * gridWidth + (x + 1)] = p;
}

inline Point DistanceField::groupCompare(Point other, const int x, const int y, const __m256i& offsets) {
    Point self = get(x, y);
...
inline Point DistanceField::singleCompare(Point other, const int x, const int y,
...
void DistanceField::generateSDF() {
    // Pass 0
